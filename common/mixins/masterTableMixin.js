 ...
 * @LastEditTime: 2021-07-06 14:09:17
 * @LastEditors: 黄婷
 * @FilePath: \misboot-cloud-vue\src\mixins\masterTableMixin.js
 */
    ...
    methods: {
        ...
        handleAdd: function(e, param) {
            ...
            if (e === 'master') {
                ...
                if (this.$refs.masterList.rows === null) {
        ...
        handleOk: function(e, paramar) {
            ...
            form.validateFields((errors, values) => {
                ...
                if (!errors) {
                    ...
                    this.$http.post(url, fromValue).then((res) => {
        ...
        handleExportXls(columns, tablesName, type, status) {
            ...
            let url = '';
            if (type === 1) {
                url = `${this.$http.config.baseUrl}${this.url.exportExcelUrl}`
            } else {
                url = `${this.$http.config.baseUrl}${this.url.exportExcelTemplate}`
            }
            if (status === '1' || status === '0') {
                window.location.href = encodeURI(url + '?tablesName=' + tablesName + '&fieldName=' + field + '&textName=' + text +
                    '&type=' + status + '&token=' + uni.getStorageSync('token'))
            } else {
                window.location.href = encodeURI(url + '?tablesName=' + tablesName + '&fieldName=' + field + '&textName=' + text +
                    '&token=' + uni.getStorageSync('token'))
            }
        ...
        handleImportExcelXS2(info) {
            ...
            if (info.file.status === 'done') {
                if (info.file.response.statusCode) {
                    if (info.file.response.statusCode === 200) {
                        let url = `${this.url.importExcelUrl}`
                        this.$http.post(url, { address: info.file.response.filePath, orgId: this.queryParam.orgId }).then((res) => {
                            console.log(res)
                            if (res.statusCode === 200 || res === 1) {
