...
http.setConfig((config) => { /* 设置全局配置 */
  // config.baseUrl = 'http://192.168.1.104:20004'
  config.baseUrl = 'http://test.whlog.cn:31000' /* 根域名不同 */
  config.header = {
    ...config.header,
  }
  if (config.custom.auth) {
	let query = window.location.href;
	uni.setStorageSync('token', query.split("?token=")[1])
	const token = uni.getStorageSync('token')
	config.header.token = token
	
	// iframe: window.addEventListener('message', function(event) {
	// 	let query = event.target.location.search.substring(1);
