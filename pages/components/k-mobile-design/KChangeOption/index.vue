	...
	<div class="option-change-container">
		<a-row v-if="type === 'option' || type === 'tab'" :gutter="8">
			<div class="option-change-box" v-for="(val, index) in value" :key="index">
				<a-col :span="9"><a-input v-model="val.label" placeholder="名称" :data-value="val.label" :data-index="index" @change="labelChange"/></a-col>
				<a-col :span="9"><a-input v-model="val.value" placeholder="值" :data-value="val.value" :data-index="index" @change="valueChange"/></a-col>
				<a-col :span="6"
...
<script>
import colorPicker from '@/pages/components/color/colorPicker.vue';
export default {
	name: 'KChangeOption',
	...
	methods: {
		labelChange(e){
			let label = e.currentTarget.dataset.value
			let index = e.currentTarget.dataset.index
			const ids = this.value.map(item=>item.label);
			const idsSet = new Set(ids);
			if(idsSet.size != ids.length){
				this.value[index].label = label
		...
		handleAdd() {
			...
			let addData = [
				...this.value,
				{
					value: `${Number(this.value[this.value.length - 1].value) + 1}`,
					label: '选项' + (Number(this.value[this.value.length - 1].value) + 1),
					list: this.type === 'tab' ? [] : undefined,
				},
			];

			// const ids = addData.map(item=>item.value);
			// const idsSet = new Set(ids);
			// if(idsSet.size != ids.length){
			// 	return this.$message.warning('请确认静态数据是否重复');
			// }
		...
		handleDelete(deleteIndex) {
			...
			this.$emit('delete',this.value[deleteIndex])
		},
