<!--
 * @Description: 表单拖拽页设计/用户设计页面
 * @Author: 黄婷
 * @Date: 2021-05-20 18:13:04
 * @LastEditTime: 2021-06-04 14:09:01
 * @LastEditors: 黄婷
 * @FilePath: \vue-antd-admine:\wwwroot\Vue\MisBoot\misboot-cloud-vue\src\components\TopVUI\k-portal-design\design.vue
-->
<template>
	<a-config-provider>
		<div :class="['form-designer-container-9136076486841527 index-container-1592076357139', { mobile: isMobile }]">
			...
			>
				...
				<aside class="left">
					...
					<a-collapse @change="collapseChange" :defaultActiveKey="collapseDefaultActiveKey">
						<!-- 布局控件 start -->
						<a-collapse-panel v-if="layoutArray.length > 0" header="布局控件" key="0">
							<collapseItem
								:list="layoutArray"
								@generateKey="generateKey"
								@handleListPush="handleListPush"
...
import { asideLeft, layoutPanel, operatingArea, layoutItemProperties, collapseItem } from './component';
import previewModal from './preview/index.vue';
import { basicsList, colList, layoutList, formList } from './config/formItemsConfig';
...
export default {
	...
	components: { asideLeft, layoutPanel, operatingArea, layoutItemProperties, previewModal, collapseItem },
	data() {
		return {
			data: {
				list: [],
	...
	watch: {
		menuData: {
			...
			handler(newVal, oldVal) {
				...
				let basicsArray = colListData.filter((item) => item.children.length != 0);
				this.basicsArray = basicsArray.filter((item) => this.fields.includes(item.type));
			},
		},
		jsonData: {
			deep: true,
